...
import qs from 'qs'
import { to } from '@/libs/utils/custom'
const apiMap = requireContext(require.context('./modules', true, /\.js$/), 'default')

export async function Api(apiPathTemp, dataTemp, headersTemp) {
  function apiBase(apiPath, data, headers) {
    ...
    return new Promise((resolve, reject) => {
      request({
        ...
        if (resp.code == 200){
          ...
          Notify({ type: 'danger', message: resp.message || resp.msg });
          reject(resp)
        }
      }).catch(err => {
  ...
  return await to(apiBase(apiPathTemp, dataTemp, headersTemp))
}
...
export default {
  install(Vue) {
    Vue.prototype.api = async function(apiPathTemp, dataTemp, headersTemp) {
      function apiBase(apiPath, data, headers) {
        const [proto, name] = apiPath.split('/')
        const apiMapReal = apiMap[proto]
        const map = {
